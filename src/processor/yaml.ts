...
import { extract_code_blocks } from './markdown.js';
...
const generate_metadata = async (
	...
	// Extract code blocks and determine categories
	const code_blocks = extract_code_blocks(content);
	const code_categories = new Set(['typescript']);
	code_blocks.forEach((block) => {
		if (block.language === 'js') code_categories.add('javascript');
		if (block.category) code_categories.add(block.category);
	});
	...
	return {
		...
		tags,
		related,
		code_categories: Array.from(code_categories),
		difficulty,
...
export const process_markdown_files = async () => {
	...
	for (const file of files) {
		try {
			...
			const escaped_content = escape_frontmatter(content);
			...
			// Merge with existing frontmatter and format it
			const final_frontmatter = format_frontmatter({
				...metadata,
